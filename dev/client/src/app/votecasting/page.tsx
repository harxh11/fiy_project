...
// --- ADD THESE IMPORTS FOR BLOCKCHAIN ---
// import { ethers } from "ethers";
// import ballotArtifact from "/Users/sadiquenadaf/Desktop/FiY_final/fiy_project/dev/client/src/app/abi/ballot1.json"; // Update path if needed
...
// import { BrowserProvider, Contract } from "ethers";
// import ballotArtifact from "../abi/Ballot.json"; // Adjust path if needed

//const CONTRACT_ADDRESS = "0xYOUR_CONTRACT_ADDRESS"; // Replace with your contract address
...
const handleConfirmVote = async () => {
  ...
  try {
    ...
    //const contract = new Contract(CONTRACT_ADDRESS, ballotArtifact.abi, signer);
    const contract = new Contract(contractInfo.address, contractInfo.abi, signer);

    // Ballot expects a proposal index (starting from 0)
    const tx = await contract.vote(selectedCandidate - 1);
    await tx.wait();

    setHasVoted(true);
    setShowConfirmDialog(false);
    setReceiptId(tx.hash);
  } catch (err) {
    alert("Voting failed: " + (err?.reason || err?.message || err));
    setShowConfirmDialog(false);
  }
};
  if (hasVoted) {
